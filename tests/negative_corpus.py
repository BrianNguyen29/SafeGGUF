...
2. True advisory regressions (promoted after triage; generated + verified)
   ...
   source: `source_url`, `ghsa` and affected/patched references recorded in
   ...
   Promoted cases also carry `fixture_origin`: "advisory-derived" when the
   fixture follows the advisory's own mechanism/example shape, or
   "mechanism-equivalent" when SafeGGUF proves only the equivalent admission
   property and does not reproduce the upstream allocation byte-for-byte
   (stated in `source`). Advisory entries whose provenance is still
   un-triaged stay in ADVISORY_PLACEHOLDERS as manifest-only records
   (fixture: null, provenance fields TRIAGE-PENDING) and are neither
   generated nor verified. Never invent tracker URLs or commit SHAs; promote
   a placeholder to a real cve-* fixture only after triage.
...
tests/fixtures/negative/manifest.json is the regenerated snapshot):
  ...
  fixture_origin     advisory-derived | mechanism-equivalent on promoted
                     cve-* cases; None for synthetic exemplars.
  ...
  affected_commit    Upstream version/commit range known to be affected; None
                     for synthetic cases, AFFECTED_COMMIT_UNSPECIFIED when the
                     triage recorded only the patched commit.
...
# Documented null policy (B3) for `affected_commit` on the promoted cve-*
# cases: the verified advisory provenance records the patched commit/build
# only - no affected commit or range was verified - and the provenance guard
# requires a triaged (non-null, non-TRIAGE-PENDING) value. Record that gap
# explicitly instead of inventing a SHA.
AFFECTED_COMMIT_UNSPECIFIED = (
    "unspecified (no affected commit verified; patched commit recorded)"
)
...
CASES = [
    ...
    # primary sources; the un-triaged registry is ADVISORY_PLACEHOLDERS).
    {
        ...
        "affected_commit": AFFECTED_COMMIT_UNSPECIFIED,
...
# source_url and the affected/patched references are recorded from a
...
def generate():
    ...
    manifest = {
        ...
        "provenance_rules": (
            ...
            "source. On promoted cve-* cases, 'fixture_origin' is "
            "'advisory-derived' (advisory mechanism/example shape) or "
            "'mechanism-equivalent' (equivalent admission property, not a "
            "byte-for-byte upstream reproduction), 'ghsa' carries the GHSA "
            "identifier, and 'affected_commit' may be the documented "
            "AFFECTED_COMMIT_UNSPECIFIED marker when the triage recorded only "
            "the patched commit. null = no advisory provenance; "
